...
/*:
 ...
 Nice, we want all modules to follow the same `view controller`, `presenter`, `interactor` structures so we gain all the advantages we talked about in all modules.

 Inside the `navigateToHome` method inside the login controller we'll setup a new home module the same way we did to the login and attach it to the view hierarchy.
